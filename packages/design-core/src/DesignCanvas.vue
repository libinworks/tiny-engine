...
const canvasUrl =
  isVsCodeEnv || isDevelopEnv
    ...
    : window.location.origin + window.location.pathname + `/canvas.html?tenant=${tenant}`
...
export default {
  ...
  setup() {
    ...
    const handleGetMessage = (e) => {
      const { data } = e
      if (data && typeof data === 'object' && data.type === 'tiny-engine-schema' && data.value) {
        // console.log('message 22:', data.value);
        const { pageState } = useCanvas()
        useCanvas().initData(data.value, pageState.currentPage)
      }
    }

    const addEventListenerMessage = () => {
      window.addEventListener('message', handleGetMessage, false)
    }

    const removeEventListenerMessage = () => {
      window.removeEventListener('message', handleGetMessage, false)
    }
